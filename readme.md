...
Запросы, требующие идемпотентности, сохраняют этот `requestId` и при повторных запросах с тем же `requestId`, игнорируют их (см. например [еще исходники](final/docker/auth_service/src/public/index.php#L149)).
...
В системе используется сага с оркестровкой, когда для исполнения саги есть специальный сервис-оркестратор,
